...
def perform_simulation(initial_conditions, vx_0, vx_ref, Y_0, Y_ref, psi_0):
    ...
    controls = results[1::20, 7:9]
    results = results[1::20, 1:7]  # 提取指定列并每隔10个点取一个点
    ...
    results = np.hstack((results, controls))  # 拼接额外的两列
    print("results=\n", results)
    return results
...
np.savetxt("Train Data with Control.csv", train_data, delimiter=",", header="X,Y,psi,vx,vy,omega_r,Y_ref,vx_ref,acc,delta_f", comments="")
np.savetxt("Test Data with Control.csv", test_data, delimiter=",", header="X,Y,psi,vx,vy,omega_r,Y_ref,vx_ref,acc,delta_f", comments="")
