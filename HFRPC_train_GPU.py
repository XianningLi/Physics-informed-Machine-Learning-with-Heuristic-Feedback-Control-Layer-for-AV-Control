import torch
from HFRPCModel.CustomLoss import CustomLoss
from HFRPCModel.IntegratedNetwork import IntegratedNetwork
from HFRPCModel.SetSeed import set_seed
import pandas as pd
...
if __name__ == '__main__':
    ...
    torch.save(model, 'HFRPC_complete_integrated_net.pth')
    torch.save(model.controller, 'HFRPC_controller_net.pth')

    # Save losses to file
    data_to_save = {
        'HFRPC_train_losses': train_losses,
        'HFRPC_test_losses': test_losses,
    }
    with open('HFRPC_losses.pkl', 'wb') as file:
        pickle.dump(data_to_save, file)

    # Save final training loss and total training time to a text file
    final_train_loss = train_losses[-1]
    total_time = time.time() - main_time_start
    with open('HFRPC_training_summary.txt', 'w') as summary_file:
    ...
    print(f'Final Training Loss and Total Time saved to "HFRPC_training_summary.txt".')
    print(f'Whole Time: {total_time:.2f} seconds')

    # Save controller model weights to .mat file
    model = torch.load("HFRPC_controller_net.pth", map_location=torch.device('cpu'))  # Load controller model for CPU
    model.eval()
    ...
    savemat("HFRPC_controller_net_weights.mat", weights, do_compression=True)

    print("\nWeights have been saved to 'HFRPC_controller_net_weights.mat'.")
