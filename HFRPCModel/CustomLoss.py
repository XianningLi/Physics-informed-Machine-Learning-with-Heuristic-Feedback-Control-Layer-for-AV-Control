...
class CustomLoss(nn.Module):
    ...
    def forward(self, states, controls, reference):
        # Extend reference to match the full state vector
        ref_extended = torch.zeros_like(states)
        batch_size, sequence_length, feature_size = ref_extended.shape
        ref_column = reference[:, 0].unsqueeze(1).expand(-1, sequence_length)  # Match sequence_length
        ref_extended[:, :, 1] = ref_column
        vx_column = reference[:, 1].unsqueeze(1).expand(-1, sequence_length)  # Match sequence_length
